...
parser.add_argument("--done-classifier", action="store_true", default=False,
                    help="Make dataset for binary termination classifier")
...
def generate_demos(n_episodes, valid, seed, shift=0):
    ...
    if args.done_classifier:
        demos_path = utils.get_demos_path(args.demos, args.env, 'agent_done', valid)
    else:
    ...
    while True:
        ...
        try:
            ...
            if done and args.include_goal:
                if "BabyAI" in env.spec.id:
                    actions.append(MiniGridEnv.Actions.done)
                    images.append(obs['image'])
                    if args.include_direction:
                        directions.append(obs['direction'])
                else:
            ...
            if reward > 0 and (args.filter_steps == 0 or len(images) <= args.filter_steps):
                if args.done_classifier:
                    if "BabyAI" in env.spec.id:
                        idxs = [np.random.randint(0, len(images)-1), len(images)-1]
                        images = [images[idx] for idx in idxs]
                        directions = [directions[idx] for idx in idxs]
                        actions = [0, 1]
                        demos.append((env.unwrapped.spec.id, mission, blosc.pack_array(np.array(images)), directions, actions))
                    else:
                        idxs = [np.random.randint(0, len(obss)-1), len(obss)-1]
                        obss = [obss[idx] for idx in idxs]
                        actions = [0, 1]
...
def generate_demos_cluster():
    ...
    if args.done_classifier:
        demos_path = utils.get_demos_path(args.demos, args.env, 'agent_done')
    else:
