...
class InstructionOnlyPreprocessor(object):
    def __init__(self, model_name, load_vocab_from=None):
        self.model_name = model_name
        self.vocab = Vocabulary(model_name)
    ...
    def __call__(self, instructions, device=None):
        raw_instrs = []
        max_instr_len = 0

        for instruction in instructions:
            tokens = re.findall("([a-z]+)", instruction.lower())
            instr = numpy.array([self.vocab[token] for token in tokens])
            raw_instrs.append(instr)
            max_instr_len = max(len(instr), max_instr_len)

        instrs = numpy.zeros((len(instructions), max_instr_len))
...
class ObjectPreprocessor(object):
    def __call__(self, obss, device=None):
        objects = numpy.array([obs["objects"] for obs in obss])
        objects = torch.tensor(objects, device=device, dtype=torch.float)
        return objects

class JointPreprocessor(object):
    def __call__(self, obss, device=None):
        pos = numpy.array([obs["pm_position"] for obs in obss])
        return torch.tensor(pos, device=device, dtype=torch.float)

class RawImageDirPreprocessor(object):
    def __call__(self, obss, device=None):
        images = numpy.array([obs["image"] for obs in obss])
        images = torch.tensor(images, device=device, dtype=torch.float)
        directions = numpy.array([obs["direction"] for obs in obss])
        directions = torch.tensor(directions, device=device, dtype=torch.float)
        directions_channel = (directions.reshape(directions.shape[0], 1, 1, 1) + 1) * (images==10).long()[...,0].unsqueeze(-1).float().to(device)
        images = torch.cat((images, directions_channel), dim=-1)
        return images
...
class ObssContPreprocessor:
    def __init__(self, model_name, obs_space=None, load_vocab_from=None):
        # self.image_preproc = RawImagePreprocessor()
        self.obj_preproc = ObjectPreprocessor()
        self.joint_preproc = JointPreprocessor()
        self.instr_preproc = InstructionsPreprocessor(model_name, load_vocab_from)
        self.vocab = self.instr_preproc.vocab
        self.obs_space = {
            # "image": 65536,
    ...
    def __call__(self, obss, device=None):
        ...
        if "objects" in self.obs_space.keys():
            obs_.objects = self.obj_preproc(obss, device=device)

        if "pm_positions" in self.obs_space.keys():
            obs_.joint_positions = self.joint_preproc(obss, device=device)
...
class ObssDirPreprocessor:
    def __init__(self, model_name, obs_space=None, load_vocab_from=None):
        self.image_dir_preproc = RawImageDirPreprocessor()
        self.instr_preproc = InstructionsPreprocessor(model_name, load_vocab_from)
        self.vocab = self.instr_preproc.vocab
        self.obs_space = {
            "image": 256,
    ...
    def __call__(self, obss, device=None):
        ...
        if "image" in self.obs_space.keys():
            obs_.image = self.image_dir_preproc(obss, device=device)
