...
class BaseAlgo(ABC):
    ...
    def __init__(self, envs, acmodel, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
                 value_loss_coef, max_grad_norm, recurrence, preprocess_obss, reshape_reward, aux_info, use_penv=False,
        ...
        if use_penv:
            logging.info("loading ParallelEnv")
            self.env = ParallelEnv(envs, use_procs=True)
            logging.info("loaded ParallelEnv")
        else:
            self.env = envs
